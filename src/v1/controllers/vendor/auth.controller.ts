...
import { vendorAuthValidations } from "../../validations/vendor/index.js";
import { ROLE } from "@prisma/client";
import { BadRequestError, ValidationError, handleResponseError } from "../../utils/errors.js";
import { CATEGORY_TYPE } from "../../types/global.types.js";
...
export const register = async (req: Request, res: Response) => {
    try {
        ...
        const { error, value:body } = vendorAuthValidations.register(req.body);
        if (error) throw new ValidationError(undefined, error.details);
        if (!isValidCategory(body.category)) throw new BadRequestError("invalid category (food | groceries)");
        ...
        const createdUser = await prisma.user.create({
            data: {
                ...
                vendor_account: {
                    create: {
                        name: body.name,
                        category: getCategory(body.category as CATEGORY_TYPE),
        ...
        // @dev Create the vendor account
        const createdAccount = await prisma.vendorAccount.findUnique({
...
export const login = async (req: Request, res: Response) => {
    try {
        ...
        return res.json({
            user: {
                firstname: _user.vendor_account!.name,
                email: _user.email
            },
            token: jwtToken
        });
